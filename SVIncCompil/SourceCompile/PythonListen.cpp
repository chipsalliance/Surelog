  ...
  if ((m_parse->m_children.size () != 0) || (m_parse->m_parent == NULL))
    {
      ...
      if ((m_parse->m_parent == NULL) && (m_parse->m_children.size () == 0))
        {
          SV3_1aPythonListener* pythonListener = new SV3_1aPythonListener (this, 
                                                                          m_compileSourceFile->getPythonInterp(), 
                                                                          m_parse->m_antlrParserHandler->m_tokens,
                                                                          0);
          m_pythonListeners.push_back(pythonListener);
          tree::ParseTreeWalker::DEFAULT.walk (pythonListener, m_parse->m_antlrParserHandler->m_tree);
              ...
              if (m_parse->m_children[i]->m_antlrParserHandler)
                {
                 ...
                 m_pythonListeners.push_back(pythonListener);
                 tree::ParseTreeWalker::DEFAULT.walk (pythonListener, m_parse->m_children[i]->m_antlrParserHandler->m_tree);
                }
            }
