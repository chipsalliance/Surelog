...
void
SV3_1aPpTreeShapeListener::logError (ErrorDefinition::ErrorType error, ParserRuleContext* ctx, std::string object, bool printColumn)
{
  ...
  if (m_pp->getMacroInfo ())
    {
      Location loc (m_pp->getMacroInfo()->m_file, m_pp->getMacroInfo()->m_line + lineCol.first -1, lineCol.second, getSymbolTable ()->getId (object));
...
void
SV3_1aPpTreeShapeListener::logError (ErrorDefinition::ErrorType error, Location& loc, bool showDuplicates)
{
  if (m_instructions.m_mute) return;
  Error err (error, loc);
  m_pp->getCompileSourceFile ()->getErrorContainer ()->addError (err, showDuplicates);
}

void
SV3_1aPpTreeShapeListener::logError (ErrorDefinition::ErrorType error, Location& loc, Location& extraLoc, bool showDuplicates)
{
  if (m_instructions.m_mute) return;
...
void
SV3_1aPpTreeShapeListener::enterComments (SV3_1aPpParser::CommentsContext * ctx)
{
  //if (m_pp->getIncluder ())
  //  return;
...
void SV3_1aPpTreeShapeListener::enterNumber(SV3_1aPpParser::NumberContext* ctx)
{
...
void
SV3_1aPpTreeShapeListener::forwardToParser (ParserRuleContext* ctx)
{
  if (m_inActiveBranch && (!m_inMacroDefinitionParsing) 
                       && (!m_pp->getCompileSourceFile()->getCommandLineParser()->filterSimpleDirectives ()) 
...
void
SV3_1aPpTreeShapeListener::checkMultiplyDefinedMacro (std::string& macroName, ParserRuleContext* ctx)
{
  ...
  if (macroInf)
    {
      std::pair<int,int> lineCol = ParseUtils::getLineColumn(m_pp->getTokenStream(), ctx);
      if ((macroInf->m_file == m_pp->getFileId (lineCol.first)) && (m_pp->getLineNb(lineCol.first) == macroInf->m_line))
        return;
      Location loc (m_pp->getFileId (lineCol.first), m_pp->getLineNb(lineCol.first), 0, getSymbolTable()->getId (macroName));
...
void
SV3_1aPpTreeShapeListener::enterInclude_directive (SV3_1aPpParser::Include_directiveContext * ctx)
{
  ...
  if (m_inActiveBranch && (!m_inMacroDefinitionParsing))
    {
      ...
      if (ctx->String ())
        ...
        {
          fileName = m_pp->evaluateMacroInstance(ctx->macro_instance()->getText(),m_pp, lineCol.first, 
                                                 PreprocessFile::SpecialInstructions::CheckLoop,
                                                 PreprocessFile::SpecialInstructions::ComplainUndefinedMacro);   
        }
      else 
        {
          Location loc (m_pp->getFileId (lineCol.first), m_pp->getLineNb(lineCol.first), 0, 0);          
      ...
      // unsigned int sectionStartLine, SymbolId sectionFile, unsigned int originalLine, unsigned int type
      ...
      PreprocessFile* pp = new PreprocessFile (fileId, m_pp, lineCol.first, m_pp->getCompileSourceFile (), m_instructions, 
                                               m_pp->getCompilationUnit(), m_pp->getLibrary ());
      ...
      if (!m_pp->m_instructions.m_filterFileLine )
        {
          ...
          if (m_pp->getCompileSourceFile ()->getCommandLineParser ()->lineOffsetsAsComments ())
            {
              pre = "/* " + pre + " */";
            }
          ...
          if (m_pp->isMacroBody () && m_pp->getMacroInfo())
          ...
          else 
            {
              if (m_pp->getCompileSourceFile ()->getCommandLineParser ()->lineOffsetsAsComments ()) 
                {
                   post = "\n/* SLline " + std::to_string(lineCol.first + 1) + " \"" + m_pp->getFileName (lineCol.first) + "\" 2 */\n";
                }
              else 
                {
                  post = "\nSLline " + std::to_string(lineCol.first + 1) + " \"" + m_pp->getFileName (lineCol.first) + "\" 2\n";
      ...
      if (pp_result != "")
        m_pp->append (pre + pp_result + post);
      ...
      IncludeFileInfo infop (lineCol.first, m_pp->getFileId (lineCol.first), m_pp->getSumLineCount ()+1 ,2);
      infop.m_indexOpening = openingIndex;
      m_pp->getSourceFile()->getIncludeFileInfo ().push_back (infop);
      if (openingIndex >= 0)
        m_pp->getSourceFile ()->getIncludeFileInfo (openingIndex).m_indexClosing = m_pp->getSourceFile ()->getIncludeFileInfo ().size()-1;
           
    }
}

void
SV3_1aPpTreeShapeListener::exitInclude_directive (SV3_1aPpParser::Include_directiveContext * ctx) {
...
void
SV3_1aPpTreeShapeListener::enterSimple_no_args_macro_definition (SV3_1aPpParser::Simple_no_args_macro_definitionContext * ctx)
{
  if (m_inActiveBranch)
    {
...
void
SV3_1aPpTreeShapeListener::exitSimple_no_args_macro_definition (SV3_1aPpParser::Simple_no_args_macro_definitionContext *ctx)
{
...
void
SV3_1aPpTreeShapeListener::enterMacroInstanceWithArgs (SV3_1aPpParser::MacroInstanceWithArgsContext * ctx)
{
  if (m_inActiveBranch)
    {
      std::string macroName;
      ...
      std::vector<tree::ParseTree*> tokens = ParseUtils::getTopTokenList (ctx->macro_actual_args ());
      ...
      if (macroInf)
        {
          ...
          IncludeFileInfo info (macroInf->m_line, macroInf->m_file, m_pp->getSumLineCount (), 1);
          m_pp->getSourceFile ()->getIncludeFileInfo ().push_back (info);
          openingIndex = m_pp->getSourceFile ()->getIncludeFileInfo ().size() -1;
          macroBody = m_pp->getMacro (macroName, actualArgs,  m_pp, lineCol.first, m_pp->getSourceFile()->m_loopChecker, 
                                      m_pp->m_instructions, macroInf->m_line, macroInf->m_file);
        }
      else 
        {     
          macroBody = m_pp->getMacro (macroName, actualArgs,  m_pp, lineCol.first, m_pp->getSourceFile()->m_loopChecker,                                    m_pp->m_instructions);
        }
              ...
              if (lineCol.second == 0)
                {
                  ...
                  pre = "SLline " + std::to_string (macroInf->m_line) + " \"" + getSymbolTable ()->getSymbol (macroInf->m_file) + "\" 0";
                  post = "SLline " + std::to_string (lineCol.first + 1) + " \"" + m_pp->getFileName (lineCol.first) + "\" 0";
                  if (m_pp->getCompileSourceFile ()->getCommandLineParser ()->lineOffsetsAsComments ())
                    {
                      pre = "/* " + pre + "*/";
                      post = "/* " + post + "*/";
                    ...
                    {
                      // Don't insert as parser does not know how to process this directive in this lexical context
                      pre = "";
                      post = "";
                    }
                }
...
void
SV3_1aPpTreeShapeListener::enterMacroInstanceNoArgs (SV3_1aPpParser::MacroInstanceNoArgsContext * ctx)
{
  if (m_inActiveBranch)
    {
      std::string macroName;
      ...
      if (macroInf)
        {
          ...
          macroBody = m_pp->getMacro (macroName, args,  m_pp, lineCol.first, m_pp->getSourceFile()->m_loopChecker, 
                                      m_pp->m_instructions, macroInf->m_line, macroInf->m_file);
        } 
      else 
        {
      ...
      if (m_pp->m_debugMacro)
        std::cout << "FIND MACRO: " << macroName << ", BODY: |" << macroBody << "|" << std::endl;
...
void
SV3_1aPpTreeShapeListener::enterPragma_directive (SV3_1aPpParser::Pragma_directiveContext * ctx)
{
  std::string type;
  if (ctx->Simple_identifier())
    type = ctx->Simple_identifier ()->getText ();
...
void
SV3_1aPpTreeShapeListener::setCurrentBranchActivity ()
{
...
bool
SV3_1aPpTreeShapeListener::isPreviousBranchActive ()
{
  PreprocessFile::IfElseStack& stack = m_pp->getStack ();
...
void
SV3_1aPpTreeShapeListener::enterSv_file_directive (SV3_1aPpParser::Sv_file_directiveContext *ctx)
{
  if (m_inActiveBranch && (!m_inMacroDefinitionParsing)) {
      if (m_pp->getMacroInfo ()) 
        {
...
void
SV3_1aPpTreeShapeListener::enterSv_line_directive (SV3_1aPpParser::Sv_line_directiveContext * ctx)
{
  if (m_inActiveBranch && (!m_inMacroDefinitionParsing))
    {
      std::pair<int, int> lineCol = ParseUtils::getLineColumn (m_pp->getTokenStream (), ctx);
      ...
      if (m_pp->getMacroInfo ()) 
        { 
          ...
          //m_pp->append (std::to_string (m_pp->getLineNb(lineCol.first) + m_pp->getMacroInfo()->m_line - 1));
...
void
SV3_1aPpTreeShapeListener::enterLine_directive (SV3_1aPpParser::Line_directiveContext *ctx)
{
  std::pair<int, int> lineCol = ParseUtils::getLineColumn (m_pp->getTokenStream (), ctx);
  std::string fileName;
  if (ctx->String())
...
void
SV3_1aPpTreeShapeListener::init ()
{
  m_reservedMacroNames = {
    "define"
    , "celldefine"
    , "endcelldefine"
    , "default_nettype"
  ...
  for (std::vector<std::string>::iterator itr = m_reservedMacroNames.begin (); itr != m_reservedMacroNames.end (); itr++)
    {
...
void
SV3_1aPpTreeShapeListener::enterDefine_directive (SV3_1aPpParser::Define_directiveContext * ctx)
{
  if (m_inActiveBranch)
    {
      std::string macroName;
      ...
      if (stringContent.find("`") != std::string::npos) 
        {
          ...
          stringContent = "\"" + m_pp->evaluateMacroInstance(stringData,m_pp, lineCol.first, 
                                                             PreprocessFile::SpecialInstructions::DontCheckLoop,
                                                             PreprocessFile::SpecialInstructions::AsIsUndefinedMacro
                                                             ) + "\"" ;
           */
          stringContent = std::regex_replace (stringContent, std::regex ("``.``"), ".");
          stringContent = std::regex_replace (stringContent, std::regex ("``-``"), "-");
        }
...
void
SV3_1aPpTreeShapeListener::enterEscaped_identifier (SV3_1aPpParser::Escaped_identifierContext * ctx)
{
  if (m_inActiveBranch && (!(m_filterProtectedRegions && m_inProtectedRegion)))
    {
      if (!m_inMacroDefinitionParsing)
