...
namespace SURELOG {
    ...
    class DataType {
    public:
        DataType() : m_fileContent(NULL), m_id(0), m_name(""), m_definition(NULL), m_is_parameter(false) {}
        DataType(FileContent* fC, NodeId id, std::string name, VObjectType type, bool isParameter = false) : 
             m_fileContent(fC), m_id(id), m_name(name), m_definition(NULL), m_type(type), m_is_parameter(isParameter) {
        }
        void init(FileContent* fC, NodeId id, std::string name, VObjectType type, bool isParameter = false) { 
             m_fileContent = fC; m_id = id; m_name = name; m_type = type; m_is_parameter = isParameter; }
        virtual ~DataType();
        ...
        void setDefinition(DataType* def) {
            m_definition = def;
        }
