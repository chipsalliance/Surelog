...
std::vector<CACHE::VObject> Cache::cacheVObjects(FileContent* fcontent,
  ...
  for (size_t i = 0; i < fcontent->getVObjects().size(); i++) {
    ...
    field1 |= 0x00000000FFF00000 & (((uint64_t)object.m_type) << (20));  // 12 Bits => Filled 32 Bits (Of 64)
    field1 |= 0x0000FFFF00000000 & (((uint64_t)object.m_column) << (20 + 12));  // 16 Bits => Filled 48 Bits (Of 64)
    field1 |= 0xFFFF000000000000 & (((uint64_t)object.m_parent << (20 + 12 + 16)));  // 16 Bits => Filled 64 Bits (Of 64) , Word Full
    field2 |= 0x00000000000000FF & (object.m_parent >> (16));  //  8 Bits => Filled  8 Bits (Of 64)
    field2 |= 0x00000000FFFFFF00 & (((uint64_t)object.m_definition) << (8));  // 24 Bits => Filled 32 Bits (Of 64)
    field2 |= 0x00FFFFFF00000000 & (((uint64_t)object.m_child) << (8 + 24));  // 24 Bits => Filled 56 Bits (Of 64)
    field2 |= 0xFF00000000000000 & (((uint64_t)object.m_sibling) << (8 + 24 + 24));  // 8/24 Bits => Filled 64 Bits (Of 64) , Word Full
    field3 |= 0x000000000000FFFF & (object.m_sibling >> (8));  //  16 Bits => Filled  16 Bits (Of 64)     
    field3 |= 0x0000000FFFFF0000 & (((uint64_t)object.m_fileId) << (16)); // 20 Bits => Filled  36 Bits (Of 64)  
    field3 |= 0x00FFFFF000000000 & (((uint64_t)object.m_line) << (16 + 20)); // 20 Bits => Filled 56 Bits (Of 64)
