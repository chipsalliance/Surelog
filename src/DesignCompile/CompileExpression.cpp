    ...
    case VObjectType::slIntConst: {
      ...
      if (tickNumber || largeInt(value)) {
        ...
        if (isSigned) {
          ...
          tps->VpiSigned(true);
          ref_obj *const ro = s.MakeRef_obj();
          ro->VpiParent(c);
          ro->Actual_group(tps);
          c->Typespec(ro);
        }
      ...
      if (Package *pack = design->getPackage(packName)) {
        if (expr *val = pack->getComplexValue(varName)) {
          ...
          if (result && (result->UhdmType() == uhdmoperation)) {
            ...
            if (ref_obj *ro = op->Typespec()) {
              opts = ro->Actual_group<UHDM::typespec>();
            }
            UHDM::ExprEval eval;
            ...
            if (substituteAssignedValue(param->Rhs(), compileDesign)) {
              if (param->Rhs()->UhdmType() == uhdmoperation) {
                ...
                if (opType == vpiAssignmentPatternOp) {
                  ...
                  const typespec *ts = nullptr;
                  if (lhs->UhdmType() == uhdmparameter) {
                    if (const ref_obj *ro = ((parameter *)lhs)->Typespec()) {
                      ts = ro->Actual_group<typespec>();
                    }
                  }
                  rhs = expandPatternAssignment(ts, (expr *)rhs, component,
  ...
  switch (parentType) {
    ...
    case VObjectType::paPort_expression: {
      ...
      while (Port_reference) {
        ...
        ref_obj *rotps = s.MakeRef_obj();
        rotps->VpiParent(op);
        rotps->Actual_group(tps);
        op->Typespec(rotps);
    ...
    if (child) {
      switch (childType) {
        ...
        case VObjectType::paCast: {
          ...
          if ((fC->Type(Simple_type) == VObjectType::paSigning_Unsigned) ||
            ...
            if (UHDM::typespec *tps =
              ...
              ref_obj *rotps = s.MakeRef_obj();
              rotps->VpiParent(operation);
              rotps->Actual_group(tps);
              operation->Typespec(rotps);
        ...
        case VObjectType::slStringConst: {
          ...
          if (sval == nullptr || (sval && !sval->isValid())) {
                              ...
                              if (lhs->UhdmType() == UHDM::uhdmtype_parameter) {
                                if (ref_obj *lhs_ts =
                                        ((UHDM::type_parameter *)lhs)
                                            ->Typespec()) {
                                  tps = lhs_ts->Actual_group<typespec>();
                                }
                        ...
                        if (substituteAssignedValue(param_ass->Rhs(),
                          ...
                          if (lhs->UhdmType() == UHDM::uhdmtype_parameter) {
                            if (ref_obj *lhs_ro =
                              ...
                              tps = lhs_ro->Actual_group<typespec>();
                            }
            ...
            if (sval->isSigned()) {
              ...
              ts->VpiSigned(true);
              ref_obj *rots = s.MakeRef_obj();
              rots->VpiParent(c);
              rots->Actual_group(ts);
              c->Typespec(rots);
            }
            result = c;
          }
        ...
        case VObjectType::paEmpty_queue: {
          ...
          ref_obj *rots = s.MakeRef_obj();
          rots->VpiParent(var);
          rots->Actual_group(s.MakeArray_typespec());
          var->Typespec(rots);
      ...
      if (Expression) {
        if (any *exp =
          ...
          if (!Constant_primary) {
            ...
            ref_obj *rotps = s.MakeRef_obj();
            rotps->VpiParent(pattern);
            rotps->Actual_group(tps);
            pattern->Typespec(rotps);
          } else {
      ...
      if (fC->Type(Constant_range) == VObjectType::paConstant_range) {
        ...
        if (rexp && (rexp->UhdmType() == uhdmref_obj) &&
          ...
          if (typespec *assoc_tps =
            ...
            ref_obj *assoc_tps_ro = s.MakeRef_obj();
            assoc_tps_ro->VpiParent(rexpc);
            assoc_tps_ro->Actual_group(assoc_tps);
            rexpc->Typespec(assoc_tps_ro);
  ...
  if (tpss->UhdmType() == uhdmstruct_typespec) {
    ...
    for (typespec_member *memb : *ts->Members()) {
      if (memb->VpiName() == suffixes[index]) {
        if (const UHDM::ref_obj *ro = memb->Typespec()) {
          result = ro->Actual_group<typespec>();
        }
    ...
    if (sig) {
      if (NodeId sigTypeId = sig->getTypeSpecId()) {
        ...
        if (fC->Type(Unpacked_dimension) != VObjectType::sl_INVALID_) {
          ...
          ref_obj *resultRef = s.MakeRef_obj();
          resultRef->VpiParent(array);
          resultRef->Actual_group(const_cast<UHDM::typespec *>(result));
          array->Elem_typespec(resultRef);
        ...
        if (netlist->ports()) {
          for (port *p : *netlist->ports()) {
            if (p->VpiName() == basename) {
              ...
              if (const UHDM::ref_obj *ro = p->Typespec()) {
...
UHDM::any *CompileHelper::compileBound(
  ...
  if (expr *operand =
    ...
    const typespec *ts = nullptr;
    if (const UHDM::ref_obj *ro = operand->Typespec()) {
      ts = ro->Actual_group<typespec>();
    }
    VectorOfrange *ranges = nullptr;
    if (ts) {
      ...
      if (instance && (reduce == Reduce::Yes)) {
        ...
        if (rootValue) {
          if (expr *expval = any_cast<expr *>(rootValue)) {
            ...
            if (ref_obj *expval_ro = expval->Typespec()) {
              ref_obj *path_ro = s.MakeRef_obj();
              path_ro->VpiParent(path);
              path_ro->Actual_group(expval_ro->Actual_group());
              path->Typespec(path_ro);
            }
...
void CompileHelper::reorderAssignmentPattern(
  ...
  if (ordered) {
    if (lhs->UhdmType() == uhdmparameter) {
      ...
      if (p->Ranges()) {
        ranges = p->Ranges();
      } else if (const ref_obj *ro = p->Typespec()) {
