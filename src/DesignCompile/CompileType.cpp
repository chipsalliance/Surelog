...
variables* CompileHelper::getSimpleVarFromTypespec(
  ...
  switch (ttps) {
    ...
    case uhdmarray_typespec: {
      ...
      if (ref_obj* atps_rt = atps->Index_typespec()) {
        if (typespec* indextps = atps_rt->Actual_group<typespec>()) {
        ...
        tpsRef->Actual_group(s.MakeArray_typespec());
      ...
      if (pname == name) {
        if (any* uparam = param->getUhdmParam()) {
          if (type_parameter* tparam = any_cast<type_parameter*>(uparam)) {
            if (const ref_obj* ro = tparam->Typespec()) {
...
UHDM::typespec* CompileHelper::compileTypespec(
  ...
  switch (the_type) {
    ...
    case VObjectType::paEnum_name_declaration: {
      ...
      ref_obj* baseTypeRef = s.MakeRef_obj();
      baseTypeRef->VpiParent(en);
      baseTypeRef->Actual_group(baseType);
            ...
            if (param_name == typeName) {
              ...
              if (const constant* exp = any_cast<const constant*>(rhs)) {
                ...
                if (const ref_obj* ro = exp->Typespec())
                  result = const_cast<typespec*>(ro->Actual_group<typespec>());
              }
              break;
            }
          }
...
bool CompileHelper::isOverloaded(const UHDM::any* expr,
  ...
  while (!stack.empty()) {
    ...
    switch (type) {
      ...
      case uhdmconstant: {
        if (const ref_obj* ro = ((constant*)tmp)->Typespec()) {
          if (const any* tp = ro->Actual_group<typespec>()) {
            stack.push(tp);
          }
        }
        break;
      ...
      case uhdmtypespec: {
        ...
        if (const ref_obj* ro = tps->Typedef_alias()) {
          if (const any* atps = ro->Actual_group<typespec>()) {
            stack.push(atps);
          }
        }
        break;
      ...
      case uhdmarray_typespec: {
        ...
        if (const ref_obj* ro = tps->Elem_typespec()) {
          if (const any* etps = ro->Actual_group<typespec>()) {
            stack.push(etps);
          }
        }
        break;
