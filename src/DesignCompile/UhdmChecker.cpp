...
float UhdmChecker::reportCoverage(const std::string& reportFile) {
  ...
  report << "\nOverall coverage: " << std::setprecision(3) << overallCoverage
         << "%\n";
  report << "\nOrdered coverage:\n";
  for (auto const& covFile : coverageMap) {
...
void collectUsedFileContents(std::set<const FileContent*>& files,
  ...
  if (instance) {
    ...
    if (def) {
      moduleNames.insert(std::string(def->getName()));
  ...
  for (const FileContent* fC : files) {
    ...
    if (!clp->createCache()) {
      if ((fileName.rfind("/bin/sv/builtin.sv") != std::string_view::npos) ||
          (fileName.rfind("uvm_pkg.sv") != std::string_view::npos) ||
          (fileName.rfind("ovm_pkg.sv") != std::string_view::npos)) {
        continue;
      }
    }
