    ...
    if (var == "this") {
      ...
      if (classDefinition) {
        ...
        for (const auto& cc : classDefinition->getBaseClassMap()) {
          currentComponent =
              valuedcomponenti_cast<const ClassDefinition*>(cc.second);
          var = "this";
          break;
        }
  ...
  switch (type) {
    ...
    case slStringConst: {
      ...
      if (def == NULL) {
        ...
        if (def) {
          ModuleDefinition* module = valuedcomponenti_cast<ModuleDefinition*>(def);
...
any* ElaborationStep::makeVar_(DesignComponent* component, Signal* sig,
  ...
  if (dtype) {
    ...
    if (const Enum* en = datatype_cast<const Enum*>(dtype)) {
      ...
      var->Typespec(spec);
      obj = var;
    } else if (/*const ClassDefinition* cl = */ valuedcomponenti_cast<
