  ...
  if (m_commandLineParser->writePpOutput() ||
    ...
    if (ofs.good()) {
      ...
      for (unsigned int i = 0; i < size; i++) {
        ...
        std::string_view fileName = m_compilers[i]->getSymbolTable()->getSymbol(
            m_compilers[i]->getFileId());
        ppFileName = FileUtils::getPreferredPath(ppFileName);
        ppFileMap[fileName.data()].push_back(ppFileName);
...
bool Compiler::createMultiProcessParser_() {
  ...
  if (m_commandLineParser->writePpOutput() ||
    ...
    if (ofs.good()) {
      ...
      for (unsigned int i = 0; i < m_compilers.size(); i++) {
        std::string_view root = m_compilers[i]->getSymbolTable()->getSymbol(
            m_compilers[i]->getFileId());
      ...
      for (CompileSourceFile* compiler : bigJobs) {
        absoluteIndex++;
        std::string_view fileName = compiler->getSymbolTable()->getSymbol(
  ...
  if (m_commandLineParser->writePpOutput() ||
    ...
    if (ofs.good()) {
      ...
      std::string outputPath =
          std::string(p).append("/").append(directory).append("../");
      std::string full_exe_path = m_commandLineParser->getExePath();
      full_exe_path = FileUtils::getFullPath(full_exe_path);
      if (m_commandLineParser->profile()) {
        full_exe_path += " -profile";
      ...
      if (nbProcesses == 1) {
        ...
        std::string command;
        command.append("cd ").append(directory).append("; cmake .; make");
        if (!muted)
          std::cout << "Running: " << command << std::endl << std::flush;
        int result = system(command.c_str());
        if (!muted)
