  ...
  if (m_commandLineParser->writePpOutput() ||
    ...
    if (ofs.good()) {
      ...
      for (CompileSourceFile* compiler : bigJobs) {
        ...
        std::string batchCmd = fileUnit + sverilog + " -parseonly -nostdout -mt 0 -mp 0 -o " + outputPath + 
                               " -nobuiltin -l " + FileUtils::basename(targetname) + ".log" + " " + svFile + fileName ;
      ...
      for (unsigned short i = 0; i < nbProcesses; i++) {
        ...
        if (!fileList.empty()) {
          ...
          std::string batchCmd = fileUnit + sverilog + " -parseonly -nostdout -mt 0 -mp 0 -o " + outputPath + 
                               " -nobuiltin -l " + FileUtils::basename(targetname) + ".log" + " " + fileList ;
      ...
      if (nbProcesses == 1) {
        ...
        ofs.close();
        std::string command = "cd " + directory + ";" + full_exe_path + " -nostdout -batch parser_batch.txt";
      ...
      if (!muted) std::cout << "FATAL: Could not create CMakeLists.txt: " << fileList << std::endl;
    }
  }
  return true;
}
  ...
  if (m_commandLineParser->writePpOutput() ||
    ...
    if (ofs.good()) {
      ...
      std::string batchCmd =
          ...
          " -nobuiltin -noparse -nostdout -cd " + std::string(p) +
          " -l " + directory + "/preprocessing.log" + " " + fileList;
      ...
      if (nbProcesses == 1) {
        ...
        ofs << batchCmd << "\n";
        ofs.close();
        std::string command = "cd " + directory + ";" + full_exe_path + " -nostdout -batch pp_batch.txt";
        if (!muted) std::cout << "Running: " << command << std::endl << std::flush;
        int result = system(command.c_str());
      ...
      std::cout << "FATAL: Could not create CMakeLists.txt: " << fileList << std::endl;
    }
  }
  return true;
}
