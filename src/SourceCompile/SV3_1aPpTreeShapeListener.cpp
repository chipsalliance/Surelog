...
void SV3_1aPpTreeShapeListener::enterInclude_directive(
  ...
  if (m_inActiveBranch && (!m_inMacroDefinitionParsing)) {
    ...
    IncludeFileInfo info(1, fileId, m_pp->getSumLineCount() + 1,
    ...
    if (!m_pp->m_instructions.m_filterFileLine) {
      ...
      if (m_pp->isMacroBody() && m_pp->getMacroInfo()) {
        ...
        if (m_pp->getCompileSourceFile()
                ...
                ->lineOffsetsAsComments()) {
          post.assign("\n/* SLline ")
              .append(std::to_string(info->m_line + lineCol.first))
              .append(" \"")
              .append(getSymbolTable()->getSymbol(info->m_file))
              .append("\" 0 */\n");
        } else {
          post.assign("\n`line ")
              .append(std::to_string(info->m_line + lineCol.first))
        ...
        if (m_pp->getCompileSourceFile()
                ...
                ->lineOffsetsAsComments()) {
          post.assign("\n/* SLline ")
              .append(std::to_string(lineCol.first + 1))
              .append(" \"")
              .append(m_pp->getFileName(lineCol.first))
              .append("\" 2 */\n");
        } else {
          post.assign("\n`line ")
              .append(std::to_string(lineCol.first + 1))
    ...
    IncludeFileInfo infop(lineCol.first, m_pp->getFileId(lineCol.first),
                          m_pp->getSumLineCount() + 1, IncludeFileInfo::POP);
  ...
  if (m_inActiveBranch && (!m_inMacroDefinitionParsing)) {
    ...
    if (macroInf) {
      IncludeFileInfo info(macroInf->m_line, macroInf->m_file,
                           m_pp->getSumLineCount() + 1, IncludeFileInfo::PUSH);
      m_pp->getSourceFile()->getIncludeFileInfo().push_back(info);
      openingIndex = m_pp->getSourceFile()->getIncludeFileInfo().size() - 1;
    ...
    if (macroInf) {
      if (!m_pp->m_instructions.m_filterFileLine) {
        if (lineCol.second == 0) {
          pre.assign("`line ")
              .append(std::to_string(macroInf->m_line))
              .append(" \"")
              .append(getSymbolTable()->getSymbol(macroInf->m_file))
              .append("\" 0");
          post.assign("`line ")
              .append(std::to_string(lineCol.first + 1))
              .append(" \"")
    ...
    if (openingIndex >= 0) {
      ...
      if (emptyMacroBody) {
        ...
        IncludeFileInfo infop(origLine, fileId, totalLineCount,
                              IncludeFileInfo::POP);
        infop.m_indexOpening = openingIndex;
        m_pp->getSourceFile()->getIncludeFileInfo().push_back(infop);
      } else {
        IncludeFileInfo infop(origLine + nbCRinArgs, fileId, totalLineCount,
                              IncludeFileInfo::POP);
        infop.m_indexOpening = openingIndex;
        m_pp->getSourceFile()->getIncludeFileInfo().push_back(infop);
      }
...
void SV3_1aPpTreeShapeListener::enterMacroInstanceNoArgs(
  ...
  if (m_inActiveBranch && (!m_inMacroDefinitionParsing)) {
    ...
    if (openingIndex >= 0) {
      ...
      if (nbCRinMacroBody) {
        IncludeFileInfo infop(line, fileId, m_pp->getSumLineCount() + 1,
                              IncludeFileInfo::POP);
        infop.m_indexOpening = openingIndex;
        m_pp->getSourceFile()->getIncludeFileInfo().push_back(infop);
        if (openingIndex >= 0)
