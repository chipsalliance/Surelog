...
SymbolId PreprocessFile::getMacroSignature() {
  ...
  return registerSymbol(macroSignature);
}
...
void PreprocessFile::recordMacro(std::string_view name, unsigned int line,
    ...
    if (loop) {
      ...
      for (auto id : loop) {
        ...
        if (macroInfo2) {
          ...
          addError(err);
          return std::make_pair(false, std::string(getCompileSourceFile()
                                                       ->getCompiler()
                                                       ->getSymbolTable()
                                                       ->getBadSymbol()));
        }
